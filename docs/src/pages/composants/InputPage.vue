...
const { t } = useI18n({
  messages: {
    fr: {
      ...
      'a11y-note':
        '<p>Pour le HTML pur, appliquez les associations ci-dessous. Les composants <code>BjInput</code> Vue et React génèrent des identifiants stables et posent ces attributs à partir des props (<code>label</code>, <code>hint</code>, <code>message</code>, <code>required</code>, état d’erreur, etc.).</p><ul class="bj-text-sm" style="margin: var(--bj-spacing-2v) 0 0; padding-left: 1.25rem"><li><strong>Obligatoire.</strong> La prop <code>required</code> expose <code>aria-required="true"</code> sur le contrôle.</li><li><strong>Libellé.</strong> Associez explicitement le libellé au champ avec <code>for</code> sur <code>&lt;label&gt;</code> et <code>id</code> sur le champ (Vue/React&nbsp;: <code>htmlFor</code> sur le composant libellé).</li><li><strong>Texte d’aide et messages.</strong> Reliez le hint et les messages au champ via <code>aria-describedby</code> (un ou plusieurs identifiants séparés par des espaces).</li><li><strong>Erreur.</strong> En état invalide, le champ porte <code>aria-invalid="true"</code>.</li><li><strong>Régions live.</strong> Le message sous le champ utilise <code>role="alert"</code> pour une erreur et <code>role="status"</code> pour un retour par défaut ou informatif.</li></ul>',
    },
    en: {
      title: 'Text input',
      ...
      'a11y-note':
        '<p>For plain HTML, wire the patterns below. Vue and React <code>BjInput</code> use stable ids and set these attributes from props (<code>label</code>, <code>hint</code>, <code>message</code>, <code>required</code>, error state, etc.).</p><ul class="bj-text-sm" style="margin: var(--bj-spacing-2v) 0 0; padding-left: 1.25rem"><li><strong>Required.</strong> The <code>required</code> prop sets <code>aria-required="true"</code> on the control.</li><li><strong>Label.</strong> Pair <code>&lt;label&gt;</code> and field with matching <code>for</code> and <code>id</code> (Vue/React: <code>htmlFor</code> on the label component).</li><li><strong>Hint and messages.</strong> Link hint and feedback text with <code>aria-describedby</code> (one or more space-separated ids).</li><li><strong>Error state.</strong> Invalid fields use <code>aria-invalid="true"</code>.</li><li><strong>Live regions.</strong> The message under the field uses <code>role="alert"</code> for errors and <code>role="status"</code> for default or informational feedback.</li></ul>',
    },
  },
})
