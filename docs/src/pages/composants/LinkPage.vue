...
const propsRows = [
  ...
  },
  {
    ...
    description: 'Corps de texte plus petit ou plus grand.',
  },
  {
    name: 'bj-link--icon-left',
    description: 'Flex avec icône avant le libellé.',
  },
  {
    name: 'bj-link--icon-right',
    description: 'Flex avec icône après le libellé.',
  },
  {
    name: 'bj-link--reset',
    description:
...
<template>
  <h1 class="bj-h1" style="margin-top: var(--bj-spacing-4v)">Lien</h1>
  <p
    class="bj-text-lg"
    style="max-width: 44rem; color: var(--bj-text-alt)"
  >
    Liens de texte soulignés aux couleurs d’action du DSBJ, avec
    variantes de taille et disposition d’icônes. Un lien ouvrant un
    nouvel onglet affiche automatiquement une icône « externe »
    lorsque <code class="bj-text-sm">target="_blank"</code> est
    utilisé.
  </p>

  <DocsSection id="exemple-link" title="Exemple">
    <DocsPreview>
  ...
  <DocsSection id="variantes-link" title="Variantes">
    <DocsPreview
      style="
        display: flex;
  ...
  <DocsSection id="classes-link" title="Classes CSS">
    <DocsPropsTable :rows="propsRows" />
    <p
      class="bj-text-sm"
      style="
        color: var(--bj-text-mention);
        margin-top: var(--bj-spacing-2v);
      "
    >
      Avec <code>target="_blank"</code>, le DSBJ ajoute une glyphe
      externe via pseudo-élément (police Remix Icon).
    </p>
  </DocsSection>

  <DocsSection id="a11y-link" title="Accessibilité">
    <DocsA11yNote>
      Les liens vers un nouvel onglet doivent l’indiquer dans le
      libellé ou un texte masqué, en complément de
      <code>rel="noopener noreferrer"</code>. Le focus visible natif
      du navigateur doit rester actif ; évitez
      <code>outline: none</code> sans alternative.
    </DocsA11yNote>
  </DocsSection>
</template>
